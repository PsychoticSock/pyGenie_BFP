...
class Graphics(BaseStruct):
    ...
    def set_graphic_item_count(_, instance: Graphics):
        ...
        Retriever.set_repeat(Graphics.graphics_data_de1, instance, instance.graphic_count)
        Retriever.set_repeat(Graphics.graphics_data_de2, instance, instance.graphic_count)
        Retriever.set_repeat(Graphics.graphics_data_swgb, instance, instance.graphic_count)
        Retriever.set_repeat(Graphics.graphics_data_aoe1, instance, instance.graphic_count)

    graphic_count: int                            = Retriever(uint16,                                            default=0, on_set=[set_graphic_item_count])
    graphic_pointers: list[int]                   = Retriever(uint32,                                            default=0)
